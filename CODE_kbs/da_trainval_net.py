...
if __name__ == '__main__':
  ...
  if args.dataset == "pascal_voc":
      ...
      args.set_cfgs = ['ANCHOR_SCALES', '[4, 8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]', 'MAX_NUM_GT_BOXES', '50']
#训练SIM10-cityspace
  elif args.dataset == "sim10k2cityspace":
      print('loading our dataset...........')
      args.s_imdb_name = "sim10k_2007_train"#用sim10k中的所有数据作为source
      args.t_imdb_name = "cityscape_2007_train_s"#用city中的source作为target
      args.s_imdbtest_name = "cityscape_2007_test_s"
      args.t_imdbtest_name = "cityscape_2007_test_t"
      args.set_cfgs = ['ANCHOR_SCALES', '[4,8,16,32]', 'ANCHOR_RATIOS', '[0.5,1,2]', 'MAX_NUM_GT_BOXES', '50']
  ...
  output_dir = args.save_dir + "/" + args.net + "/" + args.dataset+"/"+"20211213"
  ...
  if args.resume:
    ...
    '20211213_city_{}_{}.pth'.format(args.checkepoch, args.checkpoint))
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      rois_label,DA_img_loss_cls,DA_img_loss_cls_cha,tgt_DA_img_loss_cls,tgt_DA_img_loss_cls_cha,cls_loss,reg_loss=\
      ...
      loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
           + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean()\
             +(args.lamda*(DA_img_loss_cls.mean()+tgt_DA_img_loss_cls.mean())\
             +(DA_img_loss_cls_cha.mean()+tgt_DA_img_loss_cls_cha.mean())+(cls_loss.mean()+reg_loss.mean()))
      loss_temp += loss.item()
      ...
      if step % args.disp_interval == 0:
        ...
        if args.mGPUs:
          ...
          #loss_pooling=loss_pooling.mean().item()
          ...
          loss_DA_img_cha=(DA_img_loss_cls_cha.item()+tgt_DA_img_loss_cls_cha.item())/2
          cls_loss=cls_loss.item()
          reg_loss=reg_loss.item()
          fg_cnt = torch.sum(rois_label.data.ne(0))
          bg_cnt = rois_label.data.numel() - fg_cnt

        print("[session %d][epoch %2d][iter %4d/%4d]这里是saf处理city数据\n \t\t\tloss: %.4f, lr: %.2e"  % (args.session, epoch, step, iters_per_epoch, loss_temp, lr))
        print("\t\t\tfg/bg=(%d/%d), time cost: %f" % (fg_cnt, bg_cnt, end-start))
        print("\t\t\trpn_cls: %.4f, rpn_box: %.4f, rcnn_cls: %.4f, rcnn_box %.4f,\n\t\t\timg_loss %.4f,cha_loss %.4f,cls_loss %.4f,reg_loss %.4f" \
                      % (loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box,loss_DA_img_cls,loss_DA_img_cha,cls_loss,reg_loss))
    ...
    if epoch==args.max_epochs or (epoch % 1 == 0):
        save_name = os.path.join(output_dir, '20211213_city_%d_%d.pth' % (args.session, epoch))
        save_checkpoint({
